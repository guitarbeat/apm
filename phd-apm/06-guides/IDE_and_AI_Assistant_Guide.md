...
**Setup checklist**
- Configure a multi-root workspace with `phd-apm/` and the target project so quick-open (`⌘P`/`Ctrl+P`) spans both contexts.
...
**Operate**
1. Bind terminal tasks in `.vscode/tasks.json` for `python phd-apm/02-setup_review.py --help` and other repetitive commands. Chain them with `tasks.runTask` for review drills.
...
- Upload the manager initiation prompt (`04-manager-agent/manager_agent_initiation_prompt.md`) and any domain-specific `.apm` prompts as persistent references.
...
**Next steps**: Iterate on these patterns using `Customization_Guide.md`. Every improvement you discover should feed back into the prompt library or the state-handling routines so the entire PhD APM network benefits.
