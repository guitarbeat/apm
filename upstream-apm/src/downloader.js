...
export async function fetchLatestRelease(releaseTag = null, options = {}) {
  ...
  try {
    ...
    if (!options.silent) console.log(chalk.gray(`  Fetching release from: ${endpoint}`));
...
export async function fetchReleaseAssetUrl(assistant, releaseTag = null, options = {}) {
  try {
    ...
    if (!bundleFilename) {
      throw new Error(
        `Unsupported AI assistant: ${assistant}. Supported assistants: ${Object.keys(ASSET_MAP).join(', ')}`
      );
    }
    ...
    if (!asset) {
      throw new Error(
        `Bundle not found for ${assistant}. Expected file: ${bundleFilename}. This may indicate an incomplete release.`
      );
    }

    if (!options.silent)
      console.log(
        chalk.gray(`  Found asset: ${asset.name} (${(asset.size / 1024).toFixed(1)} KB)`)
      );

    return asset.browser_download_url;
  } catch (error) {
    if (!options.silent) console.error(chalk.red('Failed to fetch release asset:'));
    throw error;
  }
}
...
export async function findLatestCompatibleTemplateTag(cliVersion) {
  ...
  try {
    ...
    console.log(
      chalk.gray(`  Fetching all releases to find compatible templates for CLI v${cliVersion}...`)
    );
    ...
    console.log(
      chalk.gray(
        `  Found compatible template tag: ${latest.tag_name} (build ${latest.buildNumber})`
      )
    );
...
export async function downloadAndExtract(targetTag, assistantName, destinationPath, options = {}) {
  ...
  try {
    if (!options.silent) console.log(chalk.blue('[DOWNLOAD] Downloading assets...'));
    ...
    if (!options.silent) spinner.start('Downloading asset bundle...');
