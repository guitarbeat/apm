...
- **Customization Workflow:** Customization is now done by editing files locally within the `.apm/guides/` directory or the assistant-specific command directory _after_ running `apm init`. Acknowledged as a work-in-progress for improving flexibility.
