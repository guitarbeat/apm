...
- **Template Updates (`templates/` changes):**
  ...
  3.  The workflow reads the _current_ CLI version from `package.json` (e.g., `0.5.1`).
...
- **`apm init` (default behavior):**
  - Automatically finds and installs the latest template tag compatible with the _currently running CLI version_.
...
- **`apm init --tag <tag>` (specific version):**
  - Allows installing a _specific_ template tag by explicitly providing it (e.g., `apm init --tag v0.5.0+templates.1`).
...
- **`apm update`:**
  - Finds the latest template tag compatible with the _currently running CLI version_.
