...
export async function fetchReleaseAssetUrl(assistant, releaseTag = null, options = {}) {
  try {
    ...
    if (!options.silent) console.log(chalk.gray(`  Found release: ${release.name || release.tag_name}`));
    ...
    if (!options.silent) console.log(chalk.gray(`  Found asset: ${asset.name} (${(asset.size / 1024).toFixed(1)} KB)`));
